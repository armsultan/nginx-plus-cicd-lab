...
4.  Let's take a look at that variable in action: Open a log output
...
5. Use the web browser's FIND (Ctrl + F) function to find `$NGINX_REPO_CRT`
...
6.  **FYI and Troubleshooting only:** If the CI Pipeline fails due to invalid 
