...
2.  Does this file have `FROM alpine:3.10`? This should have `FROM
    alpine:3.9` because we want to pull dependencies from the `Alpine
    3.9` version.

    ![Screenshot of Dockerfile](./media/image6.png)

3.  Go ahead and `FROM alpine:3.10` to ` FROM alpine:3.9`, then save
...
4.  Open or switch back to GitHub Desktop Client, and you will now see
    the changes made.
...
5.  In the lower-left of the GitHub, Desktop Client provide a Commit
...
6.  Open up the **nginx-plus-dockerfiles** repository on
...
7.  On this **nginx-plus-dockerfiles** repository page, you will see the
...
8.  After clicking on the pipeline status icon, we can view the full
...
9.  A Docker Container Registry is integrated into GitLab, and every
