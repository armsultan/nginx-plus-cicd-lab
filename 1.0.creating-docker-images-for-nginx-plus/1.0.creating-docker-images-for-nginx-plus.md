...
generated](./media/image1.png){width="6.5in" height="4.1875in"}
...
generated](./media/image2.png){width="6.5in" height="4.03125in"}
...
![](./media/image4.png "Inserting image..."){width="6.5in"
height="3.75in"}

4.  Take a look at the **alpine3.9** folder, and in here, you will find
    a Dockerfile there at the top of each Dockerfile. You will see the
    line starting with the command \`**FROM**.\` The first \`**FROM**\`
    command is a critical Docker command, allowing you to pull
    dependencies from other images. You typically find the next command
    sets the maintainer of the image, for reference and information. For
    more information check out the [dockerfile
    reference](https://docs.docker.com/engine/reference/builder/)

5.  Does this file have \` FROM alpine:3.10\`? This should have \` FROM
    alpine:3.9\` because we want to pull dependencies from the Alpine
    3.9 version.\
    ![A screenshot of a computer screen Description automatically
    generated](./media/image5.png){width="6.5in"
    height="3.7395833333333335in"}

6.  Go ahead and \`FROM alpine:3.10\` to\` FROM alpine:3.9\`, then save
    the file. You can navigate to file -\> collect, or you can use the
    shortcut, **CTRL+S**. (NOTE: Do not change the file name. Keep it
    the same.)
...
7.  Open or switch back to GitHub Desktop Client, and you will now see
    the changes made.

The GitHub Desktop Client has automatically tracked the changes that
were made the Dockerfile and is highlighting those exact changes,
\`**FROM alpine3.9\`** replaced with** \`FROM alpine3.10\`**

We are now going to push these changes to our repository on GitLab.

![A screenshot of a social media post Description automatically
generated](./media/image7.png){width="6.5in" height="5.125in"}

8.  In the lower-left of the GitHub, Desktop Client provide a Commit
    title and notes before we can commit to master. 

You can see two boxes. Go ahead and type in the title field: \"**Updated
Alpine 3.9\"**. 

Additionally, you can provide notes as well, type in the notes field:
\"**Set correct base image**.\"
...
![A screenshot of a cell phone Description automatically
generated](./media/image8.png){width="6.5in" height="5.15625in"}

9.  Open up the **nginx-plus-dockerfiles** repository on
    Gitlab, **https://gitlab.f5demolab.com/f5-demo-lab/
    nginx-plus-dockerfiles** or using
...
![A screenshot of a cell phone Description automatically
generated](./media/image9.png){width="6.5in" height="3.78125in"}

10. On this **nginx-plus-dockerfiles** repository page, you will see the
    pipeline status icon next to the latest **Commit SHA**. When the
    pipeline currently in progress, you will see an **orange
    icon** (waiting to start) or **blue circle** (running). Hopefully,
    we don't see a **red icon**, which indicates the pipeline has
    failed. We can click on the pipeline status icon to view the
    pipeline progress

![A screenshot of a cell phone Description automatically
generated](./media/image10.png){width="6.5in" height="3.4375in"}
...
![A screenshot of a computer Description automatically
generated](./media/image12.png){width="6.5in" height="3.28125in"}

12. A Docker Container Registry is integrated into GitLab, and every
    project can have its own space to store its Docker images. We can
    see our image in the **Container Registry**: **Packages \> Container
    Registry**. On the Container Registry page, expand the title, and
...
getting exposed in logs. There are two types of variables in GitLab

1.  [**[Predefined environment
    variables]{.underline}**](https://docs.gitlab.com/ee/ci/variables/#predefined-environment-variables):
    GitLab CI/CD has a [default set of predefined
    variables](https://docs.gitlab.com/ee/ci/variables/predefined_variables.html) that
...
2.  [Cust**[om environment
    variables]{.underline}**](https://docs.gitlab.com/ee/ci/variables/#custom-environment-variables):
    When your use case requires a specific custom variable, you can [set
    them up quickly from the
    UI](https://docs.gitlab.com/ee/ci/variables/#creating-a-custom-environment-variable) or
    directly in the .gitlab-ci.yml file and reuse them as you wish.
    There are two variable types:

    -   [Variable
        type](https://docs.gitlab.com/ee/ci/variables/#variable-type):
...
1.  Navigate to the
    Gitlab **[[nginx-plus-dockerfiles]{.underline}](https://gitlab.f5demolab.com/f5-demo-lab/nginx-plus-dockerfiles) repository \>
...
**Note that **we cannot set these values with a **Masked** type because
our NGINX PEM files do not meet
the [requirement](https://docs.gitlab.com/ee/ci/variables/#masked-variables) of
being in the Base64 alphabet (RFC4648) format. We want to make sure we
do not echo that value in any of the Pipeline logs

> ![A screenshot of a cell phone Description automatically
> generated](./media/image13.png){width="6.5in" height="3.96875in"}

2.  Open a log output of a recent successful pipeline job and you will
    see the variables in use:

**echo "NGINX\_REPO\_CRT" \> "etc/ssl/nginx/nginx-repo.crt"**\
**echo "NGINX\_REPO\_KEY" \> "etc/ssl/nginx/nginx-repo.key"**

![A screenshot of a cell phone Description automatically
generated](./media/image14.png){width="6.5in" height="3.375in"}

3.  If the CI Pipeline fails due to invalid or expired licenses, we will
...
1.  Open an SSH Shell to the Staging Server. Find the Staging component
    in our UDF Deployment menu \> Access \> Web Shell

![A screenshot of a computer Description automatically
generated](./media/image16.png){width="6.5in"
height="4.979166666666667in"}

2.  The web shell will open as \`**root\`** user. Change to the user
    \`**centos**\` and make sure you are in in the provided project
    folder with NGINX Plus configurations
    in **\`/home/centos/nginx-plus-docker-base\`**

\`\`\`bash

\[root\@docker \~\]\# su centos

\[centos\@docker root\]\$ whoami

centos

\[centos\@docker \~\]\$ cd \~/nginx-plus-docker-base
...
3.  Pull an image from our private docker registry using the **\`docker
    pull\` **command. Run the following command below to pull our NGINX
    Plus image built on **Alpine Linux 3.10**, if you want to run
    another image you can copy the Docker image location from the Gitlab
    Container registry page from the
    n**ginx-plus-dockerfiles **repository
...
\[centos\@docker nginx-plus-docker-base\]\$ docker ps
...
0.0.0.0:9000-\>80/tcp, 0.0.0.0:9080-\>8080/tcp sleepy\_colden
...
\[centos\@docker nginx-plus-docker-base\]\$ curl <http://127.0.0.1:9000>
-H \"Host \" -L
...
7.  We can now stp\[ the container using **\`docker stop \[CONTAINER
    ID\]\`**. To find the **\`CONTAINER ID\`** again, run **\`docker
    ps\`** first
...
\[centos\@docker nginx-plus-docker-base\]\$ docker stop c3dbc2f22505
