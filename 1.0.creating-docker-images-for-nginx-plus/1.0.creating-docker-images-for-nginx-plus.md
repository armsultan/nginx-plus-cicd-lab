...
3.  Pull an image from our private docker registry using the `docker pull` **command. Run the following command below to pull our NGINX
    ...
    ```bash
    [centos@docker ~]$ docker pull registry.gitlab.f5demolab.com/f5-demo-lab/nginx-plus-dockerfiles:alpine3.10
...
7.  After finishing testing, we can now stop the container using `docker stop [CONTAINER ID]`.
    ...
    [centos@docker nginx-plus-docker-base]$ docker stop 6593b1384585
    c3dbc2f22505
    ```
