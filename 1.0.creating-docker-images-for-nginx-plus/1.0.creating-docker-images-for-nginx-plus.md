...
4.  Take a look at the **alpine3.10** folder, and in here, you will find
...
6.  Go ahead and `FROM alpine:3.10` to ` FROM alpine:3.9`, then save
    ...
    ![A screenshot of a the Dockerfile](./media/image5.png)
...
12.  A Docker Container Registry is integrated into GitLab, and every
    ...
    see our image in the **Container Registry**: **Packages** \> **Container Registry**. 
...
3.  Pull an image from our private docker registry using the `docker pull` **command. Run the following command below to pull our NGINX
...
6.  We can now test the NGINX Plus container by making a HTTP request
    ...
    ```bash
    [centos@docker nginx-plus-docker-base]$ curl http://127.0.0.1:9000 -L
    Status code: 200
...
7.  We can now stop the container using `docker stop [CONTAINER ID]`.
    To find the `CONTAINER ID` again, run `docker ps`.
...
-------------
### Navigation
> [<= Previous](../0.Introduction/0.Introduction.md) | 
> [Next =>](../2.0.deploying-nginx-plus-web-server-with-cicd/2.0.deploying-nginx-plus-web-server-with-cicd.md)